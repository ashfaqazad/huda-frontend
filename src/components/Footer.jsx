...
export default function Footer() {
  ...
  // Sync language with URL and i18n
  useEffect(() => {
    if (lng && ['en', 'ja'].includes(lng)) {
      i18n.changeLanguage(lng);
    } else {
      const defaultLng = i18n.language.startsWith('ja') ? 'ja' : 'en';
      i18n.changeLanguage(defaultLng);
    }
  }, [lng, i18n]);

  // Normalize language based on i18n state
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <ul className="space-y-2">
            <li><Link to={`/${currentLanguage}`} className="hover:text-green-400 transition">{t('navbar_home')}</Link></li>
            <li><Link to={`/${currentLanguage}/cars`} className="hover:text-green-400 transition">{t('navbar_cars')}</Link></li>
            <li><Link to={`/${currentLanguage}/about`} className="hover:text-green-400 transition">{t('navbar_about')}</Link></li>
            <li><Link to={`/${currentLanguage}/contact`} className="hover:text-green-400 transition">{t('navbar_contact')}</Link></li>
          </ul>
        </div>
